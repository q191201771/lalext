...
- [x] **All-platform**
  - [x] support linux/macOS/windows multi-system development, debugging, running
  - [x] Support multi-arch running. For example, amd64/arm64/arm32/ppc64le/mipsle/s390x.
  - [x] Support cross-compilation. You can compile executables for other platforms on any platform.
  - [x] Run without dependencies. The executable file does not have any environment or library installation dependency, and can run independently with a single file.
  - [x] provide executables for all platforms, which can be run directly without compilation (provided that all the source code is open).
  - [x] support docker. support amd and arm architecture
...
- [x] **Multiple live streaming encapsulation protocols**.
  - [x] Support RTMP, RTSP(RTP/RTCP), HLS, HTTP[S]/WebSocket-FLV/TS, GB28181
  - [x] Support conversion between different encapsulation protocols.
  - [x] Supports secondary development through plug-ins, support for the expansion of custom protocols, input and output can be.
- [x] **Multiple encoding formats**. Video support H264/AVC, H265/HEVC, audio support AAC, G711A/G711U
- [x] **Multi-format recording**. Support FLV, long MPEGTS, HLS recording (HLS live and recording can be turned on at the same time)
- [x] **HTTPS**. Support HTTPS-FLV, HTTPS-TS, HLS over HTTPS pull streaming
- [x] **WebSocket**. Support Websocket-FLV, WebSocket-TS pull streams
- [x] **TLS/SSL**. All protocols support corresponding TLS/SSL encrypted transmission. For example, RTMPS, RTSPS, HTTPS, WebSockets
- [x] **HLS**. Support LIVE real-time broadcasting and VOD full list broadcasting. Sliced files support multiple deletion methods. Support memory slicing. Support counting the number of HLS players
- [x] **RTSP**. Support over TCP(interleaved mode). Support basic/digest auth authentication. Support 461.
...
- [x] **HTTP API Interface**. Used to get information about a service and send commands to the service.
...
- [x] **Listening port multiplexing**. HTTP-FLV, HTTP-TS, HLS can use the same ports. over HTTPS similarly
- [x] **Second on playback**. gop buffering
- [x] **Support (on server-side) automatic overlay of muted audio**.
- [x] **Support pulling streams before pushing streams**
- [x] **Support reconnect after push stream disconnect, no perception on pull stream side**.
- [x] **Support custom debug format stream recording, stream playback, quickly reproduce and locate the problem**.
- [x] **Bring your own Web UI page**.
- [x] **Support video with B-frames**.
...
lalserver supports 3 ways to install and run:

#### ✒ Way 1, compile and install the source code.

Download the source code:

```shell
git clone https://github.com/q191201771/lal.git
Compile: ```shell git clone
...
> If you don't have Go compiler installed, you can refer to ["Installing GoLang on CentOS or macOS"](https://pengrl.com/p/34426/), and for windows OS you can search for tutorials on your own online.
...
With the rich HTTP interfaces provided by lalserver, business parties can easily customise the business system to fit their own logic using their own familiar language while keeping lalserver running independently.
...
In the lal project, in addition to `/app/lalserver`, which is the more core service, there are some additional small applications provided in the `/app/demo` directory, such as push and pull streaming clients, as well as a pressure testing tool, a streaming analysis tool, and a scheduling example program for the lalserver cluster.
These demos you can both use directly and show you how the stack provided by lal can be used.
...
- WeChat：q191201771
- QQ: 191201771
- WeChat group: Add my WeChat as a friend, tell me to pull you into the group!
- QQ group: 635846365
...
QR code to add the author of WeChat as friend:
